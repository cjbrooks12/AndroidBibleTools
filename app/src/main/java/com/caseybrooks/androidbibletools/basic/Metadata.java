...
public class Metadata {
			...
			if(key.equals(KEY_REFERENCE)) {
				...
				if(lhs == null || rhs == null) {
					throw new NullPointerException(
							"Cannot compare objects when one or both are null"
					);
				}
					...
					throw new ClassCastException(
							"Objects are not of the same Class: " + lhs.getClass()
							                                           .toString() + " " + rhs.getClass()
							                                                                  .toString()
					);
				}
			}
	...
	/**
	 * Sort an AbstractVerse according to multiple criteria. In the event that all metadata values
