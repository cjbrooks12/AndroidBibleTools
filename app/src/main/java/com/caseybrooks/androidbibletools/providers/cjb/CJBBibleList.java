...
public class CJBBibleList extends BibleList<CJBBible> implements Downloadable, Response.Listener<String>, Response.ErrorListener {
	...
	public void download(OnResponseListener listener) {
		...
		String tag = "CJBBibleList";
		String url = "https://heroku-website-cjbrooks12.c9.io/verse_api/bibles";
	...
	public void onResponse(String response) {
		...
		try {
			JSONArray biblesJSON = new JSONArray(response);

			bibles = new HashMap<>();

			for(int i = 0; i < biblesJSON.length(); i++) {
				JSONObject bibleJSON = biblesJSON.getJSONObject(i);
				CJBBible bible = new CJBBible();
				bible.setId(bibleJSON.getString("id"));
				bible.setService(bibleJSON.getString("service"));
				bible.setAbbreviation(bibleJSON.getString("abbreviation"));
				bible.setName(bibleJSON.getString("name"));
				bible.setNameEnglish(bibleJSON.getString("nameEnglish"));
				bible.setLanguage(bibleJSON.getString("language"));
				bible.setLanguageEnglish(bibleJSON.getString("languageEnglish"));
